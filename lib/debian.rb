Capistrano::Configuration.instance(:must_exist).load do
  ...
  namespace :debian do
    ...
    task :bash_profile do
      ...
      if yes(question)
        u = ask "Update bash_profile for which user? (default: #{user})"
        u = user if u.empty?
        upload_from_erb 'bash_profile', "/home/#{u}/.bash_profile", binding, :chown => u
      end
    end
    ...
    task :locales do
      ...
      if yes(question)
        upload_from_erb 'locale.gen', '/etc/locale.gen', binding, :chown => 'root', :chmod => '0644'
    ...
    task :iptables do
      ...
      if yes(question)
        upload_from_erb 'iptables.rules', '/etc/iptables.rules', binding
    ...
    task :sshd_config do
      ...
      if yes(question)
        ...
        upload_from_erb 'sshd_config', '/etc/ssh/sshd_config', binding, :chown => 'root', :chmod => '0644'
    ...
    namespace :aptitude do
      ...
      desc 'Aptitude build-essential'
      task :essential do
        sudo 'aptitude install build-essential -q -y'
      end
    end
  end
